...
import { hasMainTreeAttributeEffect, hasMainTreeAffinityEffect, hasMainTreeModifierGrant } from '../utils/skillEffectAggregator.js';
...
export function createSkillTreeShare(skillTree) {
  ...
  // the generated nodes with known effects (numeric attributes, offhand affinity,
  // modifier grants like the Melee/Ranged Mastery paragon nodes) so shared
  // builds compute the same attribute/affinity/paragon numbers as a save import.
  const mainNodes = (skillTree.mainTree ?? [])
    .filter(skill => skill.rowName && (skill.level ?? 1) > 0
      && (hasMainTreeAttributeEffect(skill.rowName)
...
export function skillTreeShareToData(st) {
  ...
  for (const rowName of st.mh || []) {
    if (!rowName || !(hasMainTreeAttributeEffect(rowName)
